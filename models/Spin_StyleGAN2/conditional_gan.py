...
class conditional_gan(keras.Model):
    def __init__(self, enc_block_count, latent_dim, conditional_dim, style_dim, image_size, noise_image_res):
        super().__init__()

        #---------------------------------------------
        self.use_aux = True
 
        self.generator = generator.create_generator(enc_block_count, latent_dim + conditional_dim, style_dim, noise_image_res, image_size[-1])
        ...
        self.a_loss_metric = keras.metrics.Mean(name="a_loss")
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape: 
            ...
            a_loss = self.a_loss_fn(conditional_labels, real_predictions[1])
            d_loss = fake_loss + real_loss + gradient_penalty(real_conditional_images, real_predictions, 5.0) + 5.0 * a_loss
        ...
        self.a_loss_metric.update_state(a_loss)

        return {"d_loss": self.d_loss_metric.result(), "g_loss": self.g_loss_metric.result(), "a_loss": self.a_loss_metric.result()}
...
class train_callback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if (epoch % self.plot_period) == 0:
            ...
            random_vectors, noise_images = sample_generator_input(images_count, self.enc_block_count, self.latent_dim, self.noise_image_res, t=0.0)
            
            latent_vectors = [tf.concat([random_vector, conditional_labels], axis=1) for random_vector in random_vectors]
